...
const LeavingStatus = (props) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            var result = await ApiCallGet('/Get_LeavingStatus');
            if (result.error) {
    ...
    const handleformSubmit = async (e) => {
        ...
        try {
            ...
            var result1 = await ApiCallPost("/Insert_LeavingStatus", { leaving_desc: value });
            if (result1.error) {
    ...
    const update = async (oldRow, newRow) => {
        ...
        let validate = true;
        if (newRow.leaving_desc === '' || newRow.leaving_desc === null || newRow.leaving_desc === undefined) {
            validate = false;
            Toast("Leaving Status  Cannot be empty", "error");
        }

        if (validate === true) {

            const data = {
                leaving_id: newRow.leaving_id,
                leaving_desc: newRow.leaving_desc,
            }

            console.log(data);

            const result1 = await ApiCallPost('/Update_LeavingStatus', { ...data });
            if (result1.error) {
                Toast(result1.text, "error");
            } else {
    ...
    return (
        <>
            <div className="w-100" style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <MatTable title="Leaving Status" columns={columns} style={{ margin: '1rem' }}
                    actionsAtStart={true}
                    data={record}
                    Options={options}
                    onUpdate={update} />
